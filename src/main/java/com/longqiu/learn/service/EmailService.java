...
import com.longqiu.learn.entity.EmailInfo;
...
public class EmailService {
    ...
    public boolean send(String title,String text,String toMail){
        ...
        emailInfo.setMsgFrom("longxqiu@163.com");//申请的邮箱
    ...
    public boolean send(EmailInfo emailInfo) {
        try {
            ...
            if (null != emailInfo.getAttachmentList()) {
                for (Map.Entry<String, Object> entry : emailInfo.getAttachmentList().entrySet()) {
            ...
            if (null != emailInfo.getImagesList()) {
                for (Map.Entry<String, Object> entry : emailInfo.getImagesList().entrySet()) {
                    if (entry.getValue() instanceof File) {
